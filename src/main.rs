...
fn main() {
    // 使用默认选项，不设置窗口大小，避免与不同版本的eframe不兼容
        ...
        fonts.font_data.insert(
            ...
            egui::FontData::from_owned(
                std::fs::read("C:\\Windows\\Fonts\\msyh.ttc")
                    .unwrap_or_else(|_| vec![]) // 读取失败则返回空向量
            ),
        );
        if !fonts.font_data.get("msyh").map_or(true, |f| f.font.is_empty()) {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        match result {
            OperationResult::Success(message) => {
                ...
                self.status_message = Some(message + " (Please select folder/file again to refresh view)");
